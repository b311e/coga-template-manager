...
if [[ ":$PATH:" != *":$BIN_PATH:"* ]]; then
    ...
    echo "  manifest (or manifest-*) <command> [options] - Manage template manifests and registry"
    echo "  validate <file-path>                - Validate OOXML files and generate reports"
    echo ""
    echo "Examples:" 
    echo "  create excel-book-template Book"
    echo "  manifest-generate jbc  # or: manifest generate agency jbc"
    echo "  manifest-validate      # or: manifest validate"
